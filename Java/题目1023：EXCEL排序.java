...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (scanner.hasNext()) {
			...
			for (int i = 0; i < num; i++) {
				Student student = new Student(seq ,scanner.next(), scanner.next(), scanner.nextInt());
				students[i] = student;
			}
			Arrays.sort(students);
			System.out.println("Case "+count+":");
			for (int i = 0; i < students.length; i++) {
				System.out.println(students[i].getStudentId()+" "+students[i].getName()+" "+students[i].getNum());
			}
			count += 1;
		}
		System.out.println();

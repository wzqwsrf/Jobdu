...
public class Prob1086 {
    ...
    private static void dijkstra(long[][] cost, long[] minCost,
                                 int[] visit, int n, int s) {
        minCost[s] = 0;
        visit[s] = 1;
        for (int i = 1; i < n + 1; i++) {
            ...
            for (int j = 1; j < n + 1; j++) {
                if (visit[j] == 0 && minCost[j] > (minCost[minj] + cost[minj][j])
                        && minCost[minj] != Long.MAX_VALUE && cost[minj][j] != Long.MAX_VALUE) {
